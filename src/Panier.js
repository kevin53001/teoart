  ...
  React.useEffect(() => {
    const init = async () => {
      try {
        ...
        // Construire le snapshot des remises pour le webhook
        const remisesDetail = [];
        if (reductions.tauxIllus > 0) remisesDetail.push({ type: 'illustrations', label: reductions.explicationIllus, taux: reductions.tauxIllus, economie: +(reductions.totalIllusBrut - reductions.totalIllus).toFixed(2) });
        if (reductions.tauxLivres > 0) remisesDetail.push({ type: 'livres', label: reductions.explicationLivres, taux: reductions.tauxLivres, economie: +((reductions.totalLivresBrut - reductions.totalLivres) + (reductions.totalReliesLivresBrut - reductions.totalReliesLivres)).toFixed(2) });
        if (reductions.tauxRecueils > 0) remisesDetail.push({ type: 'recueils', label: reductions.explicationRecueils, taux: reductions.tauxRecueils, economie: +((reductions.totalRecueilsBrut - reductions.totalRecueils) + (reductions.totalReliesRecueilsBrut - reductions.totalReliesRecueils)).toFixed(2) });
        if (reductions.totalLivresAvecRelieBrut > 0) remisesDetail.push({ type: 'pdf_avec_relie', label: 'PDF inclus avec version reliée (−75%)', taux: 0.75, economie: +(reductions.totalLivresAvecRelieBrut - reductions.totalLivresAvecRelie + reductions.totalRecueilsAvecRelieBrut - reductions.totalRecueilsAvecRelie).toFixed(2) });
        if (reductions.tauxBadgeTotal > 0) remisesDetail.push({ type: 'badge', label: reductions.explicationBadge?.texte || `Badge −${Math.round(reductions.tauxBadgeTotal * 100)}%`, taux: reductions.tauxBadgeTotal, economie: +reductions.remiseBadge.toFixed(2) });
    ...
    init();
  }, [montantCentimes, infos.email]); // eslint-disable-line react-hooks/exhaustive-deps
